...
client = OpenAI(
    ...
    api_key=os.getenv("DEEPSEEK_API_KEY"),  # Lê a chave do .env
)
...
# Geração do prompt para MEDITAÇÃO
def gerar_meditacao():
    return """
Você é um guia de meditação no estilo da Thais Galassi. Crie uma meditação profunda, fluida e emocional, com aproximadamente 5 minutos de duração (cerca de 30-40 linhas).

**Instruções:**
1. **Comece com um convite suave** para o ouvinte fechar os olhos e se conectar com a respiração.
2. Use **linguagem sensorial** (imagens de natureza, luz, calor, paz).
3. Inclua **momentos de pausa** (ex: "Agora, sinta...", "Permita-se...")
4. Trabalhe **emoções sutis**: aceitação, gratidão, amor-próprio, conexão divina.
5. **Finalize com um toque de luz** - uma mensagem de esperança ou despertar.

**Estilo:**
- Frases curtas e fluidas, como um sussurro no coração.
- Metáforas de luz, água, vento, árvores, céu.
- Toques de hipnose suave (ex: "Cada respiração te leva mais fundo...")
- Sem tópicos ou listas - apenas um fluxo contínuo.

**Exemplo de estrutura:**
"Feche os olhos... Sinta o peso do seu corpo... A cada respiração, você se solta mais... Imagine uma luz dourada... etc."
"""

# Geração do prompt padrão para outros temas
...
def gerar_conteudo():
    if tema == "Meditação":
        prompt = gerar_meditacao()
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "Você é um guia de meditação no estilo da Thais Galassi."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.9,  # Mais criatividade para meditação
        )
        conteudo = response.choices[0].message.content
        return f"## 🧘‍♀️ **Meditação Guiada**\n\n{conteudo}"
    else:
        ...
        if linhas:
            ...
            if len(partes) > 1:
                ...
                if len(resumo_e_texto) == 2:
                    resumo = resumo_e_texto[0].strip()
                    texto_completo = resumo_e_texto[1].strip()
                    return f"## 🧠 **{titulo}**\n\n### RESUMO\n\n{resumo}\n\n---\n\n### TEXTO COMPLETO\n\n{texto_completo}"
        return conteudo
